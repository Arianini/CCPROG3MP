...
public class Floor3EldenController{
    private Floor3EldenModel model;
    private Floor3EldenView view;
    //private CharacterModel Cmodel;
    private Floor2EldenController floor2EldenController;
    

    public Floor3EldenController(Floor3EldenModel model, Floor3EldenView view, Floor2EldenController floor2EldenController) {
        this.model = model;
        this.floor2EldenController = floor2EldenController;
        
        if (view == null) {
            this.view = new Floor3EldenView(model, this);
        } else {
            this.view = view;
        }
    ...
    public void interactWithTile() {
        ...
        } else if (currentTile instanceof CreditsTile) {
            handleCreditInteraction(); // Show credits screen for CreditsTile
        }
    }
    
    public void handleDoorInteraction() {
        if (floor2EldenController != null) {
            view.dispose(); // Dispose current Floor 3 view
            floor2EldenController.showFloor2(); // Show Floor 2 view
        } else {
    ...
    public void handleCreditInteraction(){
        view.dispose();
        SwingUtilities.invokeLater(() -> {
        CreditsTileView creditsView = new CreditsTileView();
        CreditsTileController creditsTileController = new CreditsTileController(creditsView);
